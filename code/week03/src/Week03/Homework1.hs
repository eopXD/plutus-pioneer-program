...
-- or if beneficiary2 has signed the transaction and the deadline has
-- passed.
...
mkValidator dat () ctx
  ...
  where
    ...
    checkGraber :: Bool
    checkGraber =
        beneficiary1 dat `elem` txInfoSignatories info &&
        to (deadline dat) `contains` range

    checkGiver:: Bool
    checkGiver =
        beneficiary2 dat `elem` txInfoSignatories info &&
        from (deadline dat) `contains` range
